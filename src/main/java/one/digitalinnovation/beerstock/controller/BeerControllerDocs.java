...
public interface BeerControllerDocs {
    ...
    @ApiOperation(value = "Increment the amount at stock of a beer found by a given valid Id")
    @ApiResponses(value = {
            @ApiResponse(code = 204, message = "Beer stock increased with the given amount"),
            @ApiResponse(code = 422, message = "Beer stock exceeded limit."),
            @ApiResponse(code = 404, message = "Beer with given id not found.")
    })
    BeerDTO incrementById(@PathVariable Long id, QuantityDTO quantityDTO) throws BeerNotFoundException, BeerStockExceededException;

    @ApiOperation(value = "Decrement the amount at stock of a beer found by a given valid Id")
    @ApiResponses(value = {
            @ApiResponse(code = 204, message = "Beer stock decreased with the given amount"),
            @ApiResponse(code = 422, message = "Beer does not have enough amount to decrement."),
            @ApiResponse(code = 404, message = "Beer with given id not found.")
    })
    BeerDTO decrementById(@PathVariable Long id, QuantityDTO quantityDTO) throws BeerNotFoundException, BeerStockInsufficientException;
}
